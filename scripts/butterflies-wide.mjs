// Capture wider view of butterflies.
...
const out = process.argv[2] ?? '/tmp/butterflies-wider.png';
...
// Find a butterfly and center on it
const targetX = await ev(`(() => {
  const bf = window.__scene.butterflies;
  if (!bf) return 2800;
  const visible = bf.butterflies.filter(b => b.sprite.alpha > 0.05);
  if (visible.length === 0) return 2800;
  return visible[0].sprite.x;
})()`);
const targetY = await ev(`(() => {
  const bf = window.__scene.butterflies;
  if (!bf) return 2150;
  const visible = bf.butterflies.filter(b => b.sprite.alpha > 0.05);
  if (visible.length === 0) return 2150;
  return visible[0].sprite.y;
})()`);
console.error('[bf target]', targetX, targetY);
