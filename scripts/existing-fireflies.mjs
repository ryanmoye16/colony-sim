// Capture existing fireflies (atmosphere.ts system) to assess visual quality.
...
const out = process.argv[2] ?? '/tmp/existing-fireflies.png';
...
// 21:00 = peak fireflies
await ev('window.__sim.setTick(1260)');
await ev('window.__sim.setSpeed(0)');  // freeze
await wait(500);

// Zoom into forest area
await ev(`(() => {
