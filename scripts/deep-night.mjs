// Capture deep night to assess darkness level.
...
const out = process.argv[2] ?? '/tmp/deep-night.png';
...
const log = (...a) => console.error('[night]', ...a);
...
// 02:00 = tick 120 (deep night)
await ev('window.__sim.setTick(120)');
await ev('window.__sim.setSpeed(0)');
await wait(500);

// Zoom to forest area to see stars/fireflies against dark
await ev(`(() => {
