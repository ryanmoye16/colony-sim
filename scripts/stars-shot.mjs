// Capture stars against water/sand (less obstructed by trees).
...
const out = process.argv[2] ?? '/tmp/stars.png';
...
const log = (...a) => console.error('[stars]', ...a);
...
// Zoom out fully so we see the camera viewport
await ev(`(() => {
  const cam = window.__cam.cam;
  cam.setZoom(1);
  cam.centerOn(2048, 2048);
  window.__cam.update = () => {};
})()`);
await wait(1500);
